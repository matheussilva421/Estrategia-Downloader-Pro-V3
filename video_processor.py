...
class VideoProcessor(BaseCourseProcessor):
        ...
        try:
            ...
            if not self.skip_video:
                ...
                if video_url:
                    ...
                    progress_callback = self._make_progress_callback(file_name, "video")
    ...
    async def _download_mapa_mental(
        ...
        try:
            ...
            # ✅ Callback de progresso
            progress_callback = self._make_progress_callback(file_name, "mapa_mental")
            ...
            await verify_download(file_path, logger, expected_extension='.pdf', min_size=5120)
            
            with open(file_path, 'rb') as f:
                header = f.read(512)
                if b'<!DOCTYPE' in header or b'<html' in header:
                    raise ValueError("Arquivo baixado é HTML de erro, não PDF")
            ...
            logger.warning(f"   ⚠ Erro ao baixar mapa mental: {e}")
    ...
    async def _download_resumo(
        ...
        try:
            ...
            # ✅ Callback de progresso
            progress_callback = self._make_progress_callback(file_name, "resumo")
            ...
            logger.warning(f"   ⚠ Erro ao baixar resumo: {e}")
            if 'file_path' in locals() and file_path.exists():
                try:
                    file_path.unlink()
                except:
    ...
    async def _download_slides(
        ...
        try:
            ...
            # ✅ Callback de progresso
            progress_callback = self._make_progress_callback(file_name, "slides")
            ...
            logger.warning(f"   ⚠ Erro ao baixar slides: {e}")
            if 'file_path' in locals() and file_path.exists():
                try:
                    file_path.unlink()
                except:
