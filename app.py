"""
Interface Gráfica Principal - Estratégia Downloader Pro v3.2 - VERSÃO FINAL
...
class StrategyDownloaderApp(ctk.CTk):
    ...
    LOG_PROCESS_INTERVAL = 100       # ms (durante download)
    LOG_PROCESS_INTERVAL_IDLE = 500  # ms (quando ocioso)
    MAX_LOG_LINES = 500              # Máximo de linhas no log
    PROGRESS_THROTTLE_MS = 200       # Mínimo entre updates por arquivo
    AUTO_REMOVE_DELAY = 2000         # ms para remover item concluído
    MAX_PROGRESS_EVENTS_PER_TICK = 60
    MAX_LOG_EVENTS_PER_TICK = 12
    MAX_UI_PROGRESS_RENDERS_PER_TICK = 20
    
    def __init__(self):
        super().__init__()
        
        # Configurações da janela
        self.title("🦉 Estratégia Downloader Pro v3.2")
    ...
    def _process_log_queue(self):
        ...
        try:
            ...
            interval = self.LOG_PROCESS_INTERVAL if self._is_downloading else self.LOG_PROCESS_INTERVAL_IDLE
            self.after(interval, self._process_log_queue)
    ...
    def _flush_pending_progress_updates(self):
        ...
        now = time.monotonic()
        min_interval = self.PROGRESS_THROTTLE_MS / 1000.0
        rendered = 0
        ...
        for file_name, data in list(self._pending_progress_updates.items()):
            ...
            if now - last_render < min_interval:
                continue
            if not self._should_update_progress(file_name):
                continue
    ...
    def _should_update_progress(self, file_name: str) -> bool:
        """Define se um arquivo pode ser renderizado agora (throttle)."""
        if not file_name:
            return False

        current_time = time.time() * 1000
        last_update = self._last_progress_update.get(file_name, 0)

        if (current_time - last_update) >= self.PROGRESS_THROTTLE_MS:
            self._last_progress_update[file_name] = current_time
            return True
    ...
    def _handle_progress_update(self, data):
        ...
        if total > 0 and current >= total:
             ...
             self.after(self.AUTO_REMOVE_DELAY, lambda: self._remove_download_item(file_name))
    ...
    def _remove_download_item(self, file_name):
        ...
        if file_name in self.active_downloads:
            try:
                ...
                self._last_progress_update.pop(file_name, None)
            except Exception as e:
                pass
    ...
    def _log_message(self, message, tag="INFO"):
        ...
        try:
            ...
            if lines > self.MAX_LOG_LINES:
                delete_count = min(100, lines - self.MAX_LOG_LINES + 50)
