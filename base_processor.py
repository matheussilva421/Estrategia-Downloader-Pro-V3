...
class BaseCourseProcessor:
    ...
    def _make_progress_callback(self, file_name: str, content_type: str = "video"):
        """
        Cria callback de progresso thread-safe.
        
        Args:
            file_name: Nome do arquivo sendo baixado
            content_type: Tipo de conteúdo (video, mapa_mental, resumo, slides)
            
        Returns:
            Função callback
        """
        def progress_callback(current: int, total: int, speed: float) -> None:
            if self.log_queue:
                try:
                    ...
                    # Silently drop if full to avoid blocking
                    pass
